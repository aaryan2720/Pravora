...
export default function CartReviewPage({ params }) {
  ...
  const [cartItems, setCartItems] = useState([
    { ...mockMenuItems[0], qty: 2 },
    { ...mockMenuItems[1], qty: 1 },
    { ...mockMenuItems[4], qty: 2 },
  ]);
  ...
  const handlePlaceOrder = async () => {
    ...
    toast.success('🔥 Order sent to kitchen! T1 cooking starts now.');
              ...
              {cartItems.map(item => (
                <div key={item.id} className="flex items-center gap-4 p-4 rounded-xl bg-slate-900 border border-slate-800">
                  ...
                  <div className="flex items-center gap-2.5 flex-shrink-0">
                    <button onClick={() => updateQty(item.id, -1)} className="w-7 h-7 rounded-full bg-slate-800 border border-slate-700 text-slate-400 hover:text-white font-bold flex items-center justify-center transition-all">−</button>
        ...
        <div className="p-4 border-t border-slate-900 bg-slate-950/95 backdrop-blur-sm sticky bottom-0 z-30">
          <div className="max-w-lg mx-auto flex items-center gap-3">
            ...
            <Button
              ...
              disabled={sending}
              className="px-6 flex items-center gap-2 shadow-[0_0_20px_rgba(245,158,11,0.25)]"
            >
              {sending ? (
                'Sending...'
              ) : (
